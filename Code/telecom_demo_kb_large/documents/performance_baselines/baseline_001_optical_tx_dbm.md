# Performance Baseline Note: optical_tx_dbm

## Baseline definition

The synthetic baseline for optical_tx_dbm on PE-MUL-002 is based on a rolling operational window. Typical central behavior is around -1.0, while the upper normal range approaches 2.0. Investigation should begin around -3.0, with context from the current workload.
...
For network RCA, combine optical_tx_dbm with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
