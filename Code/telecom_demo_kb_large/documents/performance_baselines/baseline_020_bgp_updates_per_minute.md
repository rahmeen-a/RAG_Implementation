# Performance Baseline Note: bgp_updates_per_minute

## Baseline definition

The synthetic baseline for bgp_updates_per_minute on EDGE-MUL-031 is based on a rolling operational window. Typical central behavior is around 80, while the upper normal range approaches 500. Investigation should begin around 1500, with context from the current workload.
...
For network RCA, combine bgp_updates_per_minute with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
