# Performance Baseline Note: memory_percent

## Baseline definition

The synthetic baseline for memory_percent on EDGE-RAW-014 is based on a rolling operational window. Typical central behavior is around 34, while the upper normal range approaches 62. Investigation should begin around 78, with context from the current workload.
...
For network RCA, combine memory_percent with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
