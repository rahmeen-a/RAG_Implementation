# Performance Baseline Note: output_discards_per_5min

## Baseline definition

The synthetic baseline for output_discards_per_5min on EDGE-PES-047 is based on a rolling operational window. Typical central behavior is around 0, while the upper normal range approaches 2. Investigation should begin around 10, with context from the current workload.
...
For network RCA, combine output_discards_per_5min with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
