# Performance Baseline Note: input_errors_per_5min

## Baseline definition

The synthetic baseline for input_errors_per_5min on PE-PES-046 is based on a rolling operational window. Typical central behavior is around 0, while the upper normal range approaches 1. Investigation should begin around 5, with context from the current workload.
...
For network RCA, combine input_errors_per_5min with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
