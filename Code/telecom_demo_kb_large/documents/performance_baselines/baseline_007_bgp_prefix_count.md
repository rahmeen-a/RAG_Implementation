# Performance Baseline Note: bgp_prefix_count

## Baseline definition

The synthetic baseline for bgp_prefix_count on EDGE-RAW-014 is based on a rolling operational window. Typical central behavior is around 400000, while the upper normal range approaches 950000. Investigation should begin around 1100000, with context from the current workload.
...
For network RCA, combine bgp_prefix_count with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
