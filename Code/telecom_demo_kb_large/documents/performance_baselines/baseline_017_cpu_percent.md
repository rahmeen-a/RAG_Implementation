# Performance Baseline Note: cpu_percent

## Baseline definition

The synthetic baseline for cpu_percent on EDGE-PES-071 is based on a rolling operational window. Typical central behavior is around 28, while the upper normal range approaches 55. Investigation should begin around 75, with context from the current workload.
...
For network RCA, combine cpu_percent with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
