# Performance Baseline Note: packet_loss_percent

## Baseline definition

The synthetic baseline for packet_loss_percent on RR-LAH-010 is based on a rolling operational window. Typical central behavior is around 0, while the upper normal range approaches 0.5. Investigation should begin around 2.0, with context from the current workload.
...
For network RCA, combine packet_loss_percent with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
