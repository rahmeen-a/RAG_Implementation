...
The synthetic baseline for optical_rx_dbm on EDGE-RAW-022 is based on a rolling operational window. Typical central behavior is around -7.0, while the upper normal range approaches -2.0. Investigation should begin around -9.0, with context from the current workload.
