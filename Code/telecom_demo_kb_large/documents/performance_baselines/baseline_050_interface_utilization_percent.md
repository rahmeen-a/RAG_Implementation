# Performance Baseline Note: interface_utilization_percent

## Baseline definition

The synthetic baseline for interface_utilization_percent on EDGE-RAW-014 is based on a rolling operational window. Typical central behavior is around 25, while the upper normal range approaches 70. Investigation should begin around 85, with context from the current workload.
...
For network RCA, combine interface_utilization_percent with interface errors, link state, BGP/IGP changes, route counts, packet loss, latency, and recent changes when available. A single metric rarely provides enough evidence.
